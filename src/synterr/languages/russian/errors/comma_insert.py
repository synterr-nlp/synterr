...
_FROZEN_PHRASES: dict[str, list[tuple[str, ...]]] = {
    ...
    # §87.3's canonical inventory, minus multi-word idioms («ни за что ни
    # про что», «ни с того ни с сего») — the matcher takes exactly one word
    # per half. е/ё doubled where corpora write both (днём/днем).
    "ни": [
        ("слуху", "духу"),
        ("бе", "ме"),
        ("больше", "меньше"),
